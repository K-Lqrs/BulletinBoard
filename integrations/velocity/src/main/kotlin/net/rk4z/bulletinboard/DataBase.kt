...
class DataBase(private val plugin: BulletinBoard) {
    ...
    fun connectToDatabase(): Boolean {
        ...
        try {
            ...
            plugin.logger.error("An unknown error occurred while connecting to the SQLite database!")
            e.printStackTrace()
            return false
        }
    }
    ...
    fun createRequiredTables() {
        if (connection == null) {
            plugin.logger.error("Could not create the required tables because the connection to the SQLite database is null!")
            return
        }
        ...
        try {
            ...
            plugin.logger.error("Could not create the required tables!")
            e.printStackTrace()
        } catch (e: Exception) {
            plugin.logger.error("An unknown error occurred while creating the required tables!")
            e.printStackTrace()
        }
    }
    ...
    fun closeConnection() {
        try {
            if (connection != null) {
                connection?.close()
                plugin.logger.info("Successfully closed the SQLite database connection!")
            }
        } catch (e: SQLException) {
            plugin.logger.error("Could not close the SQLite database connection!")
            e.printStackTrace()
        } catch (e: Exception) {
            plugin.logger.error("An unknown error occurred while closing the SQLite database connection!")
            e.printStackTrace()
        } finally {
            connection = null
        }
...
//>--------------------------------------------------------------------------------------------------<
    ...
    fun insertPost(post: Post, isOldData: Boolean = false) {
        ...
        try {
            connection?.prepareStatement(insertSQL)?.use { statement ->
                ...
                plugin.logger.info("Post successfully inserted into the database.")
            }
        } catch (e: SQLException) {
    ...
    fun deletePostFromAll(id: String) {
        val deleteSQL = "DELETE FROM posts WHERE id = ?"
        ...
        try {
            connection?.prepareStatement(deleteSQL)?.use { statement ->
                ...
                statement.executeUpdate()
                plugin.logger.info("Post deleted from all posts.")
            }
        } catch (e: SQLException) {
            plugin.logger.error("Could not delete post from all posts!")
            e.printStackTrace()
        }
    }
